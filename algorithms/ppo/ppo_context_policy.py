...
class PPOContextPolicy(PPOTorchPolicy):
    ...
    def loss(
        ...
        # Policy latent representation.
        pred_act = model.predict_function()
        pred_loss_func = torch.nn.NLLLoss()
        action_dim = pred_act.shape[-1]
        pred_loss = pred_loss_func(
            nn.LogSoftmax(dim=1)(pred_act.reshape(-1, action_dim)),
            train_batch[SampleBatch.ACTIONS].reshape(-1).to(torch.long)
        )
        ...
        model.tower_stats["pred_loss"] = pred_loss
