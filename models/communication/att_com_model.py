...
class AttComModel(TorchModelV2, nn.Module):
    """Attention-based communication model for fixed-number of agents."""
    ...
    def __init__(
        ...
        if isinstance(orig_space, gym.spaces.Tuple):
            self.obs_dim = orig_space[0].shape[0]
            ...
            self.obs_dim = getattr(orig_space, "child_space").shape[0]
    ...
    def forward(self, input_dict, state, seq_lens):
        obs_list = input_dict["obs"]
        obs = torch.stack(obs_list).swapaxes(0, 1)  # [B, num_agents, obs_dim]
        outputs, self._cur_value = self.model_forward(obs)
    ...
    def model_forward(self, obs):
        ...
        outputs = torch.reshape(self.action_decoder(encoded), [batch_size, num_agents * self.act_dim])
